    ...
    return(
        <div className={`fixed top-0 right-0 h-full w-80 bg-white shadow-lg p-4 flex flex-col border-l border-gray-200 z-50 transition-transform duration-300 ${
            ...
            <div className="flex justify-between items-center mb-4 border-b pb-2">
              ...
              <button className="text-gray-500 hover:text-red-500 text-2xl font-bold leading-none" onClick={onClose}>
                &times;
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-4">
            <ul>{ state.map((item, index) => (
              <li key={index}><CartItem name={item.name} qty={item.qty} price={item.price}/></li>
            ))}</ul>
          </div>
